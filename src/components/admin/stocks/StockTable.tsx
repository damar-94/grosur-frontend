...
export function StockTable({
  ...
  if (!storeSelected) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center border rounded-lg bg-muted/20">
        <p className="text-muted-foreground font-medium">No Store Selected</p>
        <p className="text-sm text-muted-foreground mt-1">
          Please select a store from the dropdown to view its inventory.
        </p>
      </div>
    );
  }
  ...
  if (products.length === 0 && !isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center border rounded-lg bg-muted/20">
        <p className="text-muted-foreground font-medium">No active products found in this store.</p>
      </div>
    );
  }
              ...
              <TableRow key={product.id}>
                ...
                <TableCell>
                  <Badge 
                    ...
                    className={!isOutOfStock && !isLowStock ? "bg-green-500 hover:bg-green-600" : ""}
                  >
                    {qty} Units
                  </Badge>
                </TableCell>
                <TableCell className="text-right">
                  <Button size="sm" variant="outline" onClick={() => onAdjustStock(product)}>
                    <Edit3 className="mr-2 h-3.5 w-3.5" /> Adjust
                  </Button>
                </TableCell>
              </TableRow>
            );
